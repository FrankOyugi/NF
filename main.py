...
symptoms_file_path = os.path.join(base_dir, 'Dataset', 'symptoms_df.csv')
...
def health_assist(dis):
    disease_description = descriptions_df[descriptions_df['Disease'] == dis]['Description'].values
    ...
    precautions_to_take = [col for col in precautions_to_take.values[0] if pd.notna(col)]
    
    medications = medications_df[medications_df['Disease'] == dis]['Medication'].values
    medications = [med for med in medications if pd.notna(med)]
...
def get_predicted_disease_value(patient_symptoms):
    ...
    return diseases_list[np.argmax(svc.predict([input_vector]))]
...
def predict():
    if request.method == 'POST':
        ...
        if invalid_symptoms:
            return render_template('index.html', invalid_symptoms=invalid_symptoms)
        ...
        my_precautions = [i for i in precautions_to_take]
        my_medications = [i for i in medications]
